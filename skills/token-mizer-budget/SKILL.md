...
Supply `paid_policy: {"builtin_uncapped_opt_in": true, "source": "local"}` to the allocator only after that verification. The allocator independently checks the private `github_models` object for an exact true opt-in and exact three-model allowlist on select/resume/admit. It rejects missing/false flags, unexpected or contradictory fields, and non-bare fresh built-in IDs. Route evidence, host availability/context and actor permission are still caller annotations, not independent host truth: verify them at dispatch. Revoke or missing policy blocks reuse. Do not let a stale, previously admitted handoff authorize the next invocation.
