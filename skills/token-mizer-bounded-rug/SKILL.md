...
2. Keep tasks needing five or fewer direct calls in the coordinator only when its context capacity and route eligibility are established. For substantial work, estimate total context first and use one suitable context-fitting builder. Reuse that worker and compact checkpoints. Large-context work requires confirmed GitHub capacity; it must block rather than silently use small-context Luna when Flash is denied. Use a fresh admission's exact `handoff.selected_runtime_id` for each host invocation; never reuse cached permission.
...
For optional typed routing evidence, first export the allocator at an explicit exclusive UTC cutoff, then use `add-assignment --assignment-export <file> --assignment-id <id>` on the matching task record. Task identity/class/boundary and event-derived snapshots must match; `attach-rug` stores reference strings only. A snapshot newer than the ledger cutoff is excluded, not treated as zero attempts or success. Use an already reconstructed allocator as-of export to preserve valid earlier observations. The report's `--rug-ledger` returns matching observations under `rug_ingestion`. This optional path does not gate ordinary routing.
